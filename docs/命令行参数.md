...
| `win7_uefi_patch` | bool | 正常端写给 PE 的兼容交接提示；仅在已确认 Windows 7 x64、可能使用 UEFI 且启用引导修复时自动置为 `true`，不提供手工选择。PE 仍会在格式化前根据实际所选 WIM/ESD 卷的版本与架构重新判定，旧配置缺失或错误的该字段不会跳过或误触发 UefiSeven。 |
