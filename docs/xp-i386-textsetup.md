...
从正常 Windows 选择当前系统盘时，桌面端会先只读校验 `I386/AMD64` 的完整性，把源目录复制到其他安全数据分区中的会话隔离目录，写入仅含安全目录分量的 PE 配置后重启。PE 会在任何 DiskPart、格式化或目标盘写入之前再次解析并校验暂存目录；非当前系统分区或已经处于 PE 时仍可直接进入下面的部署流程。
...
- 支持从正在运行的 Windows 选择当前系统盘发起重装；实际格式化和文本模式部署始终在自动进入的 PE 中执行，不在桌面会话原地覆盖。
