...
- `正常系统端/build.rs`：生成 Windows 资源、程序清单、包含 16 至 256px 多 DPI 帧的 PNG 派生 ICO 和可复现的构建日期/版本信息，并把固定 Windows 11 `Aero.msstyles` 提取的复选框/单选框明暗及 96/120/144/192 DPI 状态 PNG 转换为编译期 BGRA 图元；release 禁止测试权限 feature。
- `正常系统端/src/main.rs`：桌面端进程入口、权限与依赖检查、窗口显示前并行完成系统/硬件摘要及正式版分区只读预加载、CLI 分派、PE 安装/备份入口和原生 Win32 窗口启动；所有危险 CLI（包括历史 `/PEINSTALL`/`/PEBACKUP`）必须在管理员边界之后分派，历史 PE 安装入口只接受安全文件名、唯一会话绑定的标记/配置并在格式化前完成镜像完整性校验；网络目录由窗口异步加载并回传错误，正常端不再声明或链接 egui 模块，`non-elevated-tests` 下使用隔离互斥锁并禁止危险 CLI 入口、联网和安装目标分区预加载，但仍只读加载系统与硬件摘要供无 UAC 视觉测试。
...
- `正常系统端/src/native_ui/dialog.rs`：Inno 风格原生模态/非模态对话框壳、主动按当前明暗主题绘制且把内容容器收到的命令、通知、颜色及 `WM_HSCROLL` 原样转发给所有者、ComboBox `WM_DRAWITEM` 和 ListView 表头的内容容器、按 DFM 约 46px 基线收紧的命令栏、工具弹窗同级命令按钮保持 10px 逻辑间距、以 75px 为下限并按实际微软雅黑 UI 字体/DPI 测量译文扩宽的命令按钮、仅响应 `BN_CLICKED` 且完整抑制擦除白闪的稳定自绘按钮、已有非模态窗口激活、按显示器工作区约束的高 DPI 尺寸、长说明实测换行与内容自然高度收紧、工作区钳制和消息转发；首次显示前统一为遗漏字体的后创建子控件补充微软雅黑并预先应用 Edit/ComboBox/ListBox/ListView/按钮明暗主题，工具弹窗专用窗口类的 `hIcon/hIconSm` 固定为空且“关闭”操作只保留标题栏 X（取消操作仍保留命令按钮），主窗口独立类继续使用发布图标；异步结果不得清除尚未消费的关闭/命令结果或复活已隐藏窗口，不承载业务副作用。
...
- `正常系统端/src/native_ui/theme.rs`：固定从 Inno 6.7 Modern Windows 11 明暗参考截图逐像素核对的窗口、普通/引导按钮和边框色，负责系统应用主题检测、GDI 画刷及主题资源生命周期；单行 Edit 保留原生文本、光标、选择、IME 和无障碍语义，移除宿主版本不一致的 CLIENTEDGE 后与闭合 ComboBox 共用确定性 Win11 字段底色、悬停、焦点、禁用和圆角外框；多行滚动报告继续使用 Explorer 单层边框；闭合 ComboBox、ListView 与独立 ListBox 移除会在焦点、滚动或表头悬停后复活的矩形 `WS_BORDER`/`WS_EX_CLIENTEDGE`，改由控件自身在原生绘制完成后最后覆盖确定性 5px 圆角外框，ComboBox 必须在首个 `WM_SETCURSOR`、UxTheme 悬浮 `WM_TIMER` 和焦点/按键状态的默认处理后同步补画完整闭合选区及外框，禁止先暴露矩形原生动画帧再异步覆盖；只读的 `CBS_DROPDOWNLIST` 获得焦点后必须成对隐藏/恢复 USER32 自动创建的插入光标，避免点击瞬间出现无编辑意义的竖线；ListView 表头重绘时只在 ListView 自身坐标系补齐唯一外框，禁止把列表尺寸的圆角框画进可横向滚动的 Header DC；空表仍以当前编辑区底色完成完整绘制；ComboBox 弹出选项继续由 Common Controls 6.0/UxTheme 原生绘制，不混入 `WM_DRAWITEM` 行自绘，顶层 ComboLBox 每次展开时仅重申沉浸式明暗并显式使用原生直角弹层，不叠加圆角、边框覆盖或逐行自绘；自动复选框和单选框保留 USER32 的切换/分组、键盘、焦点、通知和无障碍语义，视觉层使用从固定 Windows 11 `Aero.msstyles` 经 UxTheme 提取并嵌入的明暗、96/120/144/192 DPI 全状态图元；提取图元必须恢复直通 alpha，并用高采样圆角覆盖率裁除小数 DPI 下主题合成产生的四角暗像素，最终尺寸匹配时直接逐像素输出，禁止再次插值造成黑角或颗粒；两类控件的标题由当前明暗调色板绘制为微软雅黑，禁止深色回退成黑色标题；`SetWindowTheme` 自己触发 `WM_THEMECHANGED`，主题入口不得再同步重复发送或在样式位未变化时强制 `SWP_FRAMECHANGED`，避免多余重绘和页面闪动；工具进度条采用确定性明暗绘制，无损扩大 C 盘滑块用兼容位图离屏完成背景、抗锯齿圆角轨道、绿色已选段和滑块后一次性发布，拖动、键盘和捕获变化只排队重绘当前控件且禁止擦除背景，只有设置位置/范围的 `TBM_*` 消息会失效控件，绘制期的 `TBM_GET*` 查询严禁递归触发同步重绘；ListView 的未选中行继续由 comctl32 原生绘制，仅在 Windows 11 v6 主题会覆盖 `clrTextBk` 的真实选中行通过官方 `NM_CUSTOMDRAW` 最小接管背景与各列文字，固定复用左侧导航按钮的静止选中配色，同时保留原生选择状态、列布局、滚动、键盘和无障碍语义，点击、悬停和键盘选择不得额外整表重绘；ComboBox 和传统 ListBox 不与该 ListView 选中色逻辑耦合，继续保持各自既有行为；无权限开发构建可用环境变量仅做明暗视觉回归。
- `正常系统端/src/native_ui/window.rs`：正常端原生窗口类、显式大/小应用图标、消息循环、Per-Monitor V2 DPI、统一 Microsoft YaHei UI 字体、受当前显示器工作区约束的 DPI 最小跟踪尺寸、国际化导航/页头/命令栏和页面布局；主窗口统一转发 ComboBox owner-draw，ListView 表头只由 `theme.rs` 的唯一入口绘制；页面切换批量暂停重绘并在显隐和布局稳定后一次性重绘全部子控件，避免暴露中间布局造成闪烁；安装页对驱动、重启、引导模式及无人值守控件做响应式分行，宽窗口中立即重启按实测译文宽度紧跟驱动下拉保持标准字段间距，取消无人值守后不再保留隐藏的文件选择和提示槽位，而是按当前译文宽度把可容纳的高级操作即时重排到引导模式之后；启用高级选项时 DiskPart 复选框、打开目录和修改引导命令按实测微软雅黑宽度紧凑排列并与条件启动签名选择保持正常组间距，启动签名标签与下拉框只保留标准字段间距；底部命令栏保留稳定内边距，硬件页隐藏刷新后把保存和复制从右向左紧密排列，复制成功只局部刷新按钮为本地化“已复制”并用非阻塞三秒定时器稳定恢复，不使用缩放、弹性动画或整页重绘；分区动态状态随语言切换重新翻译。窗口统一过滤 Button 的非点击通知，工具对话框只响应一次真实点击且已有窗口优先激活，避免焦点通知导致重复窗口和后台任务；GHO、镜像和哈希浏览内容命令直接打开文件选择器并回填原对话框，不借用会隐藏窗口的命令栏结果；工具对话框关闭结果先于异步完成消息处理，轮询存活覆盖全部专用窗口和后台任务，关闭 A 后打开 B 不得复活 A；分区对拷首次库存仍在后台只读枚举，完成后额外投递窗口唤醒消息立即消费结果，定时轮询仅作为失败安全回退，不要求用户点击刷新；系统目标在桌面默认当前系统、PE 默认首个离线系统，并按规范化 `SystemDrive` 排除与在线系统重复的离线盘符项；物理磁盘和普通数据卷等不可逆目标仍不得自动预选。窗口把原生页面意图路由到安装、备份、异步下载目录及工具控制器、带代次校验的异步只读库存和强类型后台请求边界（包括区分在线/离线目标的 APPX 请求）；在线目录刷新后以同一 PE 快照驱动下拉显示和执行映射，单一 PE 自动选中但不显示无意义选择器，安装 PE 下拉使用明暗字段主题；安装或备份在首次点击、BitLocker 解锁和 PE 下载续做后都重新枚举并复核稳定分区身份、路由及锁定状态，简易模式下载完成后重新读取真实镜像卷和架构；安装页同时保留源/目标无人值守探测、多 PE、与引导修复和实际 UEFI 条件一致的 PCA2011/PCA2023 检测与选择，自动启动签名项必须显示当前实际推荐代次，以代次及目标磁盘/分区身份隔离异步同盘 ESP 只读检测结果，并仅在高级 DiskPart 明确可能创建 ESP 时按既有规则放行缺失状态；镜像切换不得覆盖用户选择的 Win7 修复项，执行失败时仅展示本地化摘要而把底层诊断留在日志；安装、备份、下载、扩容或镜像校验仍在运行时拦截窗口直接关闭，先请求取消并等待安全停止点；工具页负责把专属一键分区与 BitLocker 对话框接入 fresh 库存、二次确认和后台安全边界，并把镜像校验的实时进度及取消标志贯通到既有校验器；Wi-Fi 迁移只在已连接网络时抓取本次运行所需的瞬态配置；无损扩大 C 盘使用专属分析、二次确认、PE 下载及交接状态机。
- 安装页主命令状态与高级选项命令栏补充约束：切回安装页时必须在控件显隐稳定后，从真实镜像、目标分区、PE、PCA 和无人值守状态完整重算“开始安装”，不得沿用页面切换时的临时禁用态或直接伪造启用；高级选项页只将唯一的“保存并返回”按内容区水平居中，不改变其他页面命令按钮的位置；复用并移动同一 owner-draw 命令按钮前必须发送 `WM_CANCELMODE` 清除旧位置的热态/按下态，并在暂停重绘完成显隐与布局后一次性恢复和刷新，禁止等待鼠标移动才清除残影。
...
- `正常系统端/src/native_ui/pages/advanced.rs`：安装高级选项的原生分组控件、条件显示、运行期完整本地化、依赖启用、Wi-Fi SSID 状态和 `AdvancedOptionsData` 双向映射；为脚本、驱动和自定义内容路径生成明确的浏览意图并接收窗口控制器回填，返回时对“已勾选但必填文本为空”的配对选项同步取消并持久化关闭，不自行打开文件对话框或执行离线系统写入。
...
- `正常系统端/src/native_ui/pages/info.rs`：把启动时已读取的 `HardwareInfo` 映射为响应式“类别/项目/值”原生 ListView，同组类别只在首行显示且类别列保持紧凑；复制与 UTF-8 TXT 导出使用清晰的分组标题和逐行“项目: 值”格式，兼容中英文标签、续行缩进和完整硬件数据，不重复输出同组类别或把多条内容粘连成一行；硬件报告占满页面主体，保存操作与复制操作一起位于稳定底部命令栏；同时承载紧凑且不侵入底部状态栏的关于页，以及语言、日志、小白模式、8/16/32 下载线程档位、WIM 引擎和高级选项设置，语言刷新按钮与闭合下拉框实际高度对齐，保留运行期完整本地化、PE 环境禁用规则、版权致谢、单一许可入口、纯按钮链接与日志目录意图，并用统一响应式按钮网格消除无意义空白，不把普通说明和 URL 伪装为编辑框或另行显示 URL 文本。
...
- `正常系统端/src/core/app_config.rs`：`config.json` 用户偏好、语言、日志、外观和默认选项的读取保存；小白模式与高级选项保持互斥并归一旧配置冲突；下载线程字段向后兼容旧配置并在加载与保存入口归一到 8/16/32 三档，旧配置保持原有 16 连接默认值。
...
- `正常系统端/src/core/install_config.rs`：正常端到 PE 的安装、备份、扩容配置，安装标记、资源暂存和无人值守文件验证；扫描覆盖 A-Z 并用 `SessionId` 唯一绑定安装标记与配置，旧空会话配置仅在单一无歧义任务时兼容，所有写入 INI 的值在落盘前拒绝 CR/LF/NUL 注入；备份与扩容交接配置使用同目录原子替换并返回可精确恢复旧文件或仅删除本次新文件的事务凭据，不删除数据目录中的其他用户文件；回滚会独立尝试恢复配置与标记并合并错误，避免单项失败阻断另一项恢复。
...
- `正常系统端/src/core/native_install_controller.rs`：原生安装页输入快照的校验、Direct/ViaPE 路由，以及保持 XP/GHO/PCA/驱动/无人值守字段兼容的安装意图与配置转换；PCA 检测只在镜像支持、启用引导修复且目标可能使用 UEFI 时阻断，不能让持久化选择误拦 GHO、XP 或 Legacy 安装。
- `正常系统端/src/core/native_install_executor.rs`：原生安装意图的 Direct/ViaPE 有序阶段计划、BitLocker/PCA/稳定目标安全门、进度取消消息和可注入副作用后端；`InstallExecutionError::user_message` 按失败类别和阶段提供不泄露底层诊断细节的本地化用户摘要，完整 code/detail 继续保留在 `Display` 日志边界，开发测试构建强制拒绝执行。
...
- `正常系统端/src/core/native_install_backend.rs`：原生安装执行器的生产后端阶段实现；复用既有镜像、格式化、驱动、引导和高级选项边界，显式保留 Wi-Fi 瞬态 profile，恢复后台 fresh BitLocker 门禁、无恢复密钥时的完全解密等待、DiskPart 后按稳定分区身份重定位/分配盘符，以及 WIM/GHO/校验长任务的实时进度与安全取消反馈；WIM/ESD/SWM 应用和校验贯穿调用方原子取消标记，libwim 可中止当前操作，取消后禁止引擎回退和后续安装阶段；主机驱动导出保持旧版可选步骤语义，失败只告警而不阻断镜像部署，并在开发测试构建中保持无副作用。
...
- `正常系统端/src/ui/advanced_options.rs`：迁移期保留高级选项的离线应用业务实现，并由 `core` 兼容接入保证 PE 安装入口不变；其中旧 egui 渲染已停编译，纯状态由 `core/ui_state.rs` 承担，后续继续拆分后删除旧文件。
...
- `PE端/src/main.rs`：PE 进程入口、文件日志、panic 记录、语言检测、BitLocker 密钥透传解锁、CLI、工作流模块声明和窗口启动；CLI 与 GUI 共用安全暂存路径解析、XML 转义无人值守生成、配置卷标和受控 DiskPart 脚本语义，禁止维护绕过这些约束的重复安装实现；P3 默认把安装、备份、扩容 GUI 路由到原生进度页，`--legacy-egui-progress`/`/LEGACYEGUIPROGRESS` 保留显式兼容回退，原生 HWND 构造失败也仅在 worker 尚未启动时回退；P2 壳仍可通过 `--native-ui-shell` 单独预览。
- `PE端/src/app.rs`：PE 安装主工作流、对用户名等动态值执行 XML 转义的共享无人值守生成和共享 `WorkflowSession`；镜像与自定义无人值守文件只能由配置数据目录和经安全文件名校验的相对文件名解析，禁止绝对路径或目录穿越；该会话集中持有单一 worker 启动门、消息接收器、进度状态、只读恢复摘要、可查询完成状态并仅在完成后回收的 worker 句柄及持久化工作流观察器，原生与兼容渲染器只消费同一状态，不得各自启动工作流；收到完成/失败消息只代表展示终态，不能在清理、延迟或重启等 worker 尾部退出前关闭进程；消息通道在未收到终态时断开必须合成本地化失败并记录 journal，避免把 worker 崩溃伪装成成功；旧 egui `App` 仅作为显式兼容入口。
...
- `PE端/src/core/config.rs`：读取正常端写入的安装、备份、扩容配置和操作标记，提供旧配置默认值；配置扫描覆盖 A-Z 并排除当前 PE 系统盘，暂存文件统一通过安全文件名校验后在数据目录内解析。
...
- `PE端/src/native_ui/mod.rs`：PE 原生 Win32 UI 的模块边界，汇总局部主题、基础控件、窗口状态、P2 主窗口壳、P3 长任务进度页、P4 只读详情页和 P5 低分辨率/DPI 纯几何层。
...
- `PE端/src/native_ui/layout.rs`：PE P5 不依赖 HWND 的纯窗口几何，计算壳、进度、详情命令栏以及 monitor work-area 夹取；除 640×440、800×600 外还以 640×440 外窗在高 DPI 下可能得到的 624×401/378 保守客户端区域覆盖 96–192 DPI，保证标题、当前步骤、总体进度、至少一行微软雅黑状态和命令栏不相交；完整步骤空间不足时由调用方进入最多两列或只保留当前步骤文字与总体进度的 compact 展示。
- `PE端/src/native_ui/progress.rs`：PE P3/P4/P5 安装、备份和无损扩容的原生 Win32 长任务与只读详情页，复用共享 `WorkflowSession` 单次启动 worker，以 50ms 定时器合并既有消息并只更新变化的文字/进度区域；动态安装/备份步骤保留无分配的静态键，仅在当前步骤或列表实际重绘时本地化，英语模式不得残留中文且轮询不得为翻译反复分配；首个真实步骤消息前显示准备态且不伪造当前步骤，无损扩容的总进度直接跟随既有百分比；完成/失败消息先进入展示终态，必须等 worker 句柄确认结束并非阻塞回收后才启用关闭，以覆盖失败清理、延迟和重启尾部；页面内导航复用同一会话并提供稳定返回/详情命令栏；低视口按纯几何隐藏副标题，空间继续不足时隐藏独立步骤百分比和步骤进度条但始终保留当前步骤文字与总体进度，长状态放入受内容区约束且至少容纳一行微软雅黑及内边距的只读滚动框，footer 空间不足时隐藏而按钮不重叠；初始窗口和 `WM_DPICHANGED` 建议矩形都夹到当前显示器工作区；消息循环异常仍等待同一 worker 真正结束；检查点、完成重启和底层错误语义保持不变。
- `PE端/src/native_ui/window.rs`：PE P2 原生 Win32 主窗口、标题与说明区、稳定底部命令栏、深浅标题栏及内容绘制、Per-Monitor V2 DPI、微软雅黑 UI、DPI 重排和受显示器工作区约束的 640×440 逻辑最小尺寸；P5 复用纯几何在低视口隐藏副标题并约束正文，初始显示和 `WM_DPICHANGED` 后都把窗口夹到当前 monitor `rcWork`，按钮按实际译文测量且不得重叠；当前仅作为显式分阶段壳，任何关闭路径都返回兼容进度入口，不执行安装、备份、扩容或磁盘操作。
- `PE端/src/ui/mod.rs`：PE UI 模块声明。
- `PE端/src/ui/advanced_options.rs`：把高级选项应用到离线系统，包括驱动、CAB、注册表、无人值守和兼容修复。
- `PE端/src/ui/progress.rs`：PE 安装/备份步骤及共享 `ProgressState` 模型，集中提供动态步骤的当前语言名称并用英语资源完整性测试覆盖全部步骤键；以显式 `has_current_step` 区分准备首帧和真实步骤，无损扩容的总进度直接采用后端百分比；同时保留旧 egui 步骤列表、进度条和错误展示供显式兼容入口，默认可见进度由 `native_ui/progress.rs` 渲染。
...
- `PE端/src/utils/i18n.rs`：PE 语言文件扫描、加载、切换、翻译和参数替换；PE EXE 内置当前 en-US 表作为缺失键兜底，WIM 中的外部 en-US 仍可覆盖同名键，使仅替换 EXE 时新增英文文案也能生效而无需修改用户管理的 WIM 语言资源。
