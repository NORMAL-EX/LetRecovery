...
- `正常系统端/src/core/bcdedit.rs`：定位和挂载 ESP、BCD/BCDBoot 修复、活动分区处理及 PCA 引导源选择；ESP 盘符分配和 MBR 活动标记必须复用共享 WinAPI 存储边界，不得生成或解析 DiskPart 文本；Legacy 修复必须同时确认 Bootsect 成功和活动位写入成功后才能报告完成。
