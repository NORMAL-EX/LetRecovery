...
- `正常系统端/src/native_ui/theme.rs` 的单行 Edit 现行职责以本节“最终发布约束补充”为准：受支持的普通不透明路径保留 USER32 的原生单边框与格式矩形，由默认过程绘制正文、选区、光标、IME 和水平滚动位置，再最后覆盖确定性圆角外框；禁止用 `WM_NCCALCSIZE` 改写客户区。仅开发门禁后的扩展玻璃实验使用完整 BGRA DIB 与原生 `WM_PRINTCLIENT`，默认过程先更新原生状态，主题层随后修复整帧 Alpha 并一次发布。不得用固定起点的 `DrawText` 替代原生 Edit 视口；`WS_EX_LAYERED`、颜色键和逐字符 `WM_SETREDRAW` 仍然禁止。
