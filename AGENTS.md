...
- 扩展玻璃客户区中的单行 Edit 必须保持普通非分层子 HWND；禁止 `WS_EX_LAYERED`、颜色键和依赖系统保存的离屏图像。USER32 继续独占文本缓冲、光标、选择、IME、键盘命中与无障碍语义；正常 `WM_PAINT` 成功路径在同一个 32 位顶向下 BGRA DIB 中填充完整字段背景并按当前文本缓冲绘制正文，`GdiFlush` 后修复整帧 Alpha，再用一次 `SetDIBitsToDevice` 发布。不得通过 `WM_NCCALCSIZE` 把 Edit 拆成中央客户区与四条非客户区带；`WM_CHAR`、`WM_UNICHAR`、`EM_REPLACESEL`、`WM_IME_COMPOSITION`、剪切、粘贴、清除、撤销和 Delete 等消息必须先交给默认过程更新原生状态，再在返回前同步发布最终帧，且禁止在此处发送 `WM_SETREDRAW(FALSE)`。DIB 创建失败时才允许原生 `WM_PRINTCLIENT` 降级。圆角仍由统一 8×8 覆盖率外框绘制，禁止二值 HRGN；首次显示、语言和主题事务不得依赖鼠标或计时器才显现。普通短 `SS_LEFT` 标签保持库存顶对齐，仅 `SS_CENTERIMAGE` 标签垂直居中。主窗口不得使用 `WS_EX_COMPOSITED`。ComboBox、ListBox 和 ListView 不套用 Edit 画刷补偿；闭合 ComboBox 的正文、箭头和外框共享同一次 normal/hot/dropped 状态，箭头使用带覆盖率的预乘 Alpha BGRA 图元。
...
- `正常系统端/src/native_ui/backdrop.rs`：正常端顶层窗口共用的实验性 Mica 边界；关闭全窗口材质时显式写入公开 `DWMSBT_AUTO` 并撤销客户区 frame，使默认标题栏继续由 DWM 决定，启用时先确认 `DwmIsCompositionEnabled`，再设置并用 `DwmGetWindowAttribute` 回读 `DWMSBT_MAINWINDOW`。已确认的 Mica 会话不随 `WM_NCACTIVATE` 反复卸载和重装：激活时以负边距扩展完整客户区，失活时把 frame 收回为零并让控件恢复普通调色板；DWM 或任一步失败时仍必须恢复 AUTO、零 frame 和普通背景。主窗口与工具对话框共用这一实现，最终树发布后可用 `DwmFlush` 等待当前合成批次，WinPE 不得发起 Mica 请求。
...
- `正常系统端/src/native_ui/controls.rs`：原生子控件创建、UTF-16 字符串、库存下拉框空选择哨兵与无偏移索引校验、按 Inno DFM 的 23px 高/75px 最小宽基线缩放的 DPI 尺寸，以及按钮、分隔线和进度条的明暗/悬停/按下/禁用/单焦点边框状态绘制；ComboBox 保持 Common Controls 6.0 的库存字符串、键盘、无障碍和原生弹层语义，不得全局强加 `CBS_OWNERDRAWFIXED` 或在每个弹出项重绘时同步读取文本和分配缓冲；所有自绘按钮在共用子类中用 `TrackMouseEvent(TME_LEAVE)` 跟踪悬停，只局部重绘自身并复用 normal/hot/pressed/disabled 调色板，跟踪失败、取消模式、隐藏复用和禁用时立即清除热态，不因焦点增加双边框；按钮、长任务进度条及闭合字段/列表外框使用限域抗锯齿绘制，圆弧厚度必须随 DPI 与直边同步，重复重绘使用绝对调色板颜色以避免颗粒、断线和逐帧变暗；全客户区 DWM 材质下，自绘按钮与圆角表面必须通过 32 位顶向下 BGRA 缓冲保留真实覆盖率 Alpha，同尺寸/半径/调色板图元使用有界线程本地缓存复用，禁止页面切换时重复构造相同 4 倍圆角位图；实体控件文字使用白色 GDI 字形遮罩重着色为预乘 Alpha；浅色材质的文本 STATIC 必须在单个兼容位图中用 Windows 11 中性材质回退色和当前文字色完整绘制后一次 `BitBlt`，不得依赖会在重定向子 HWND 中丢失文字贡献的玻璃缓冲；CPU 读取或写入经过 GDI 绘制的位图前必须调用 `GdiFlush`，禁止让延迟 GDI 批处理与像素访问交错；单行编辑框从创建期保留 `WS_CHILD|WS_VISIBLE|WS_TABSTOP|ES_AUTOHSCROLL` 和 `WS_EX_NOPARENTNOTIFY`，移除宿主版本会复活的 `WS_BORDER`/`WS_EX_CLIENTEDGE`，文本、光标、选择、IME 与无障碍仍完全由原生 Edit 负责；其 `WM_PAINT` 必须把原生 `WM_PRINTCLIENT` 绘制到 32 位 BGRA DIB，在 `GdiFlush` 后把全帧 Alpha 修复为不透明并一次 `AlphaBlend` 提交，禁止 Edit 光标、浅色黑字或首次显示暴露为透明空白中间表面；外层闭合字段视觉与 ComboBox 共享确定性 Win11 圆角框，闭合 ComboBox 箭头必须使用子像素覆盖率预乘 Alpha 图元；统一布局子类只调整真实单行 Edit 的 HWND 高度，使其按 23px 字段基线在响应式行内上下居中，直接创建的安装镜像和高级选项字段也必须接入，不得用 `EM_SETRECT` 或自绘文字伪造居中；进度条保持深色轨道、统一 Inno 绿色与 5px 内克制圆角，并在同一 BGRA 像素表面按覆盖率合成背景、外框、轨道和填充，填充圆角外部不得以矩形轨道色覆盖外框或留下黑块。
- 单行 Edit 的最终发布约束补充（覆盖上一条旧的 `WM_PRINTCLIENT + AlphaBlend` 描述）：原生 Edit 继续独占文本缓冲、光标、选择、IME、键盘命中和无障碍语义；材质模式的最终可见字段必须在同一个 32 位 BGRA DIB 中填充完整客户区并按当前原生文本缓冲绘制正文，`GdiFlush` 后把 Alpha 修复为不透明，再用一次 `SetDIBitsToDevice` 发布。禁止用 `WM_NCCALCSIZE` 把字段拆成中央客户区和四条自绘带，禁止在 `WM_CHAR`、`WM_UNICHAR`、粘贴、替换、撤销或 IME 合成后的同步最终绘制前发送 `WM_SETREDRAW(FALSE)`；后者会由系统暂时移除 `WS_VISIBLE` 并漏掉该次绘制。普通短 `SS_LEFT` 标签保持库存顶对齐，只有明确带 `SS_CENTERIMAGE` 的单行标签才垂直居中，避免激活切换时文字基线下移。
...
- `正常系统端/src/native_ui/theme.rs` 的单行 Edit 现行职责以本节“最终发布约束补充”为准：正常成功路径不得再调用 `WM_PRINTCLIENT` 生成最终正文，也不得把字段拆分成非客户区带；默认过程先更新原生状态，主题层再把完整背景与当前文本一次写入不透明 BGRA 帧。只有创建 DIB 失败的降级路径可交回原生 `WM_PRINTCLIENT`，不得伪装成材质合成成功。`WS_EX_LAYERED`、颜色键和逐字符 `WM_SETREDRAW` 仍然禁止。
...
- 正常端实验性窗口背景约束：关于页只提供一个明确标注“实验性”的 Mica 复选开关，`config.json` 的 `experimental_window_backdrop` 只接受 `none/mica`；历史 `auto/acrylic/mica_alt` 和未知值都安全回退 `none`，不得继续暴露或隐式选择其他效果。启用后主窗口与全部正常端工具对话框共用公开的 `DWMWA_SYSTEMBACKDROP_TYPE=DWMSBT_MAINWINDOW` 和负边距全客户区 DWM frame：基础层和圆角外部像素使用黑色玻璃键，交互表面使用克制的材质专用实体色；浅色深色字形、透明 STATIC、复选/单选标题、按钮、ComboBox、Header、ListView 行和空表必须分别走经过实机验证的 Alpha 安全路径，不得用普通 GDI 兼容位图、近黑色或主题发光文字冒充正确合成。控件子类引用必须同时保存明暗与材质状态，材质接受状态确定后必须同步重建字段画刷，空 ListView 的 `WM_PAINT` 必须填满完整客户区而不能仅依赖可能为空的 `rcPaint`。材质接受状态必须由 DWM 合成开启、`DWMSBT_MAINWINDOW` 设置成功和公开属性回读一致共同确认；本顶层窗口 `WM_NCACTIVATE` 失活时必须设置 `DWMSBT_AUTO`、撤销全客户区 frame 并恢复普通调色板，使结果与关闭 Mica 一致，重新激活后才重新申请。`none`、WinPE、不支持构建、回读不一致或任一 DWM 调用失败均回退普通背景；PE 端不得继承。实现及微软来源见 `docs/EXPERIMENTAL_WINDOW_BACKDROPS.md`。
- 正常端实验性窗口激活边界补充（覆盖上一条失活时写回 `DWMSBT_AUTO` 的旧描述）：配置为 `mica` 且 DWM 设置/回读已成功时，系统 backdrop 会话跨激活状态保留；失活只把 `DwmExtendFrameIntoClientArea` 的边距恢复为零并切回普通调色板，重新激活再恢复负边距完整客户区。只有关闭配置、WinPE、端点不支持或 DWM 失败才写回 `DWMSBT_AUTO`。激活事务完成后必须一次发布现有控件树并调用 `DwmFlush`；在实窗瞬时帧完成复核前，`TODO.md` 对焦点闪烁的事项不得划掉。
- `正常系统端/src/native_ui/window.rs`：正常端原生窗口类、显式大/小应用图标、消息循环、Per-Monitor V2 DPI、统一 Microsoft YaHei UI 字体、受当前显示器工作区约束的 DPI 最小跟踪尺寸、国际化导航/页头/命令栏和页面布局；主窗口统一转发 ComboBox owner-draw，ListView 表头只由 `theme.rs` 的唯一入口绘制；页面切换批量暂停重绘并在显隐和布局稳定后一次性重绘全部子控件，避免暴露中间布局造成闪烁；安装页对驱动、重启、引导模式及无人值守控件做响应式分行，宽窗口中立即重启按实测译文宽度紧跟驱动下拉保持标准字段间距，取消无人值守后不再保留隐藏的文件选择和提示槽位，而是按当前译文宽度把可容纳的高级操作即时重排到引导模式之后；启用高级选项时 DiskPart 复选框、打开目录和修改引导命令按实测微软雅黑宽度紧凑排列并与条件启动签名选择保持正常组间距，启动签名标签与下拉框只保留标准字段间距；底部命令栏保留稳定内边距，硬件页隐藏刷新后把保存和复制从右向左紧密排列，复制成功只局部刷新按钮为本地化“已复制”并用非阻塞三秒定时器稳定恢复，不使用缩放、弹性动画或整页重绘；分区动态状态随语言切换重新翻译。窗口统一过滤 Button 的非点击通知，工具对话框只响应一次真实点击且已有窗口优先激活，避免焦点通知导致重复窗口和后台任务；GHO、镜像和哈希浏览内容命令直接打开文件选择器并回填原对话框，不借用会隐藏窗口的命令栏结果；工具对话框关闭结果先于异步完成消息处理，轮询存活覆盖全部专用窗口和后台任务，关闭 A 后打开 B 不得复活 A；分区对拷首次库存仍在后台只读枚举，完成后额外投递窗口唤醒消息立即消费结果，定时轮询仅作为失败安全回退，不要求用户点击刷新；系统目标在桌面默认当前系统、PE 默认首个离线系统，并按规范化 `SystemDrive` 排除与在线系统重复的离线盘符项；物理磁盘和普通数据卷等不可逆目标仍不得自动预选。窗口把原生页面意图路由到安装、备份、异步下载目录及工具控制器、带代次校验的异步只读库存和强类型后台请求边界（包括区分在线/离线目标的 APPX 请求）；在线目录刷新后以同一 PE 快照驱动下拉显示和执行映射，单一 PE 自动选中但不显示无意义选择器，安装 PE 下拉使用明暗字段主题；安装或备份在首次点击、BitLocker解锁和 PE 下载续做后都重新枚举并复核稳定分区身份、路由及锁定状态，简易模式下载完成后重新读取真实镜像卷和架构；安装页同时保留源/目标无人值守探测、多 PE、与引导修复和实际 UEFI 条件一致的 PCA2011/PCA2023 检测与选择，自动启动签名项必须显示当前实际推荐代次，以代次及目标磁盘/分区身份隔离异步同盘 ESP 只读检测结果，并仅在高级 DiskPart 明确可能创建 ESP 时按既有规则放行缺失状态；已退役 Windows 7 UefiSeven/USB3/NVMe 资源对应的高级选项始终隐藏并在镜像切换时清零，存储驱动工具必须按当前 SetupAPI PCI ID 选出唯一 Intel VMD 子目录且禁止递归；实验性背景在 DWM 确认后按实际材质状态同步重建全部主窗字段画刷，明暗显式材质都启用负边距全客户区 frame 并在完整子控件树应用 Alpha 合成主题，失败路径必须撤销 frame 并回到不透明客户区；`WM_NCACTIVATE` 只申请一次目标 backdrop 并刷新既有子类调色板引用，不能把激活变化当作主题变化而重复 `SetWindowTheme`、强制 frame change 或重建控件；`non-elevated-tests` 的确定性安装页视觉夹具会插入三条只读虚拟分区行，仅用于验证非空列表尾部和选中行，不得进入 release 行为；执行失败时仅展示本地化摘要而把底层诊断留在日志；安装、备份、下载、扩容或镜像校验仍在运行时拦截窗口直接关闭，先请求取消并等待安全停止点；工具页负责把专属一键分区与 BitLocker 对话框接入 fresh 库存、二次确认和后台安全边界，并把镜像校验的实时进度及取消标志贯通到既有校验器；Wi-Fi 迁移只在已连接网络时抓取本次运行所需的瞬态配置；无损扩大 C 盘使用专属分析、二次确认、PE 下载及交接状态机。
  从高级选项返回时必须保留 `advanced_visible` 来源标志直到页面事务捕获，并在冻结提交期间执行完整安装页布局，使条件镜像卷行恢复到分区标题之前；`non-elevated-tests` 只允许用显式 `LETRECOVERY_UI_TEST_IMAGE_VOLUME` 注入确定性镜像卷视觉夹具，release 不得包含该入口。
...
- `正常系统端/src/core/app_config.rs`：`config.json` 用户偏好、语言、日志、外观和默认选项的读取保存；小白模式与高级选项保持互斥并归一旧配置冲突；下载线程字段向后兼容旧配置并在加载与保存入口归一到 8/16/32 三档，旧配置保持原有 16 连接默认值；正常端实验性 `experimental_window_backdrop` 只接受 `none/mica`，历史 `auto/acrylic/mica_alt` 和未知值安全回退 `none`，关于页开关通过原子配置保存入口更新该字段。
...
- `PE端/src/native_ui/layout.rs`：PE P5 不依赖 HWND 的纯窗口几何，计算壳、进度、详情命令栏、进度条右侧同排百分比、monitor work-area 居中和夹取；除 640×440、800×600 外还以 640×440 外窗在高 DPI 下可能得到的 624×401/378 保守客户端区域覆盖 96–192 DPI，保证标题、当前步骤、总体进度、步骤列表和命令栏不相交；完整步骤空间不足时由调用方进入最多两列或只保留当前步骤文字与总体进度的 compact 展示。
- `PE端/src/native_ui/progress.rs`：PE P3/P4/P5 安装、备份和无损扩容的原生 Win32 长任务页，复用共享 `WorkflowSession` 单次启动 worker，以 50ms 定时器合并既有消息并只更新变化的文字/进度区域；加载圈另用约 16ms 的局部动画时钟并只覆盖步骤列表中真实 `InProgress` 行的状态图元，不得放在顶部当前步骤标题旁或退化为 50ms 工作轮询动画；动态安装/备份步骤保留无分配的静态键，仅在当前步骤或列表实际重绘时本地化，英语模式不得残留中文且轮询不得为翻译反复分配；首个真实步骤消息前显示准备态且不伪造当前步骤，顶部只显示 `当前步骤：[名称]`，不再显示冗余的“正在安装 Windows，请勿关闭程序”说明；无损扩容的总进度直接跟随既有百分比；运行页不创建只读状态 Edit，也不显示高级选项或关闭按钮，详细诊断写入日志，底部只保留不可取消/终态文案；当前步骤和总体百分比固定在各自进度条右侧同排；初始窗口按当前显示器工作区居中，`WM_DPICHANGED` 建议矩形仍夹到工作区；窗口类和 HWND 都必须使用发布资源中的大小图标；消息循环异常仍等待同一 worker 真正结束；检查点、完成重启和底层错误语义保持不变。
- `PE端/src/native_ui/window.rs`：PE 原生 Win32 共享窗口壳、标题与说明区、稳定底部命令栏、深浅标题栏及内容绘制、Per-Monitor V2 DPI、微软雅黑 UI、DPI 重排和受显示器工作区约束的 640×440 逻辑最小尺寸，并向生产进度页提供工作区居中/夹取、资源 ID 1 大小图标加载、标题栏主题和消息清理辅助；窗口类 `hIcon/hIconSm` 与顶层 `WM_SETICON` 必须同时设置，禁止回退默认应用图标；保留的壳预览不属于运行入口且不执行安装、备份、扩容或磁盘操作。
