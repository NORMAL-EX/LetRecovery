...
- `lr-core/src/boot_pca.rs`：PCA2011/PCA2023 签名识别、固件信任评估、ESP 临时挂载、模式决策和兼容回退。
...
- `正常系统端/src/core/dism.rs`：正常端高层镜像查询、释放、捕获和进度模型，接入统一 WIM 引擎。
...
- `正常系统端/src/ui/system_install.rs`：系统安装页面、ISO/镜像选择、卷索引、模式选择、PCA 选项和安装确认。
