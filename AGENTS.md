...
- `正常系统端/src/core/native_install_backend.rs`：原生安装执行器的生产后端阶段实现；复用既有镜像、格式化、驱动、引导和高级选项边界，原版 XP/2003 当前系统盘重装会递归拒绝链接和特殊项、统计空间、把 I386/AMD64 及 x64 同级 I386 复制到会话唯一临时目录、复验后原子提交给 PE；ViaPE 安装源若与目标数据区路径相同必须按已存在源复核而非删除或复制自己，其他路径必须复制到会话唯一暂存文件、按实际字节计算校验后再原子发布。Direct 安装的 Windows 10/11 默认无人值守在写入前必须通过 `lr-core::offline_international` 读取已释放目标的国际化设置，不能从当前正常系统或 WinPE 宿主继承；显式保留 Wi-Fi 瞬态 profile，恢复后台 fresh BitLocker 门禁、无恢复密钥时的完全解密等待、DiskPart 后按稳定分区身份重定位/分配盘符，以及 WIM/GHO/校验长任务的实时进度与安全取消反馈；WIM/ESD/SWM 应用和校验贯穿调用方原子取消标记，libwim 可中止当前操作，取消后禁止引擎回退和后续安装阶段；主机驱动导出保持旧版可选步骤语义，失败只告警而不阻断镜像部署，并在开发测试构建中保持无副作用。
...
- `正常系统端/src/download/server_config.rs`：只通过固定 HTTPS `v3/index.json` 单请求获取 PE、系统镜像、软件、简易模式和显卡驱动目录，严格校验 schema、过滤禁用项并映射到既有配置契约；v3 请求、解析或结构校验失败时整体失败并保留上下文，禁止回退 v1/v2 多文件目录，也禁止把全空或部分吞错响应标记为加载成功。

### 正常系统端主要 UI

- `正常系统端/src/ui/mod.rs`：正常端 UI 模块声明，以及适配明暗主题的活动状态和警告文字公共颜色。
- `正常系统端/src/ui/about.rs`：关于页面、版本、项目链接、许可信息和支持包导出入口渲染。
- `正常系统端/src/ui/advanced_options.rs`：迁移期保留高级选项的离线应用业务实现，并由 `core` 兼容接入保证 PE 安装入口不变；Defender 选项复用共享引擎白名单移除边界且失败关闭；XP 驱动注入必须沿用 `lr-core::xp` 的失败关闭语义，不能只记录错误后继续；随包存储驱动先枚举当前 PCI 硬件 ID 并只向 DISM 传递匹配的单个 Intel VMD 目录，不再递归导入整包；其中旧 egui 渲染已停编译，纯状态由 `core/ui_state.rs` 承担，后续继续拆分后删除旧文件。
...
- `PE端/src/ui/advanced_options.rs`：把高级选项应用到离线系统，包括驱动、CAB、注册表、无人值守和兼容修复；Defender 选项复用共享引擎白名单移除边界且失败关闭；随包存储驱动必须先按 PE 当前 SetupAPI PCI 硬件 ID 选择 Intel VMD 目录，未匹配、包缺失或 AMD/Apple/VirtIO 控制器均不得跨过 DISM 边界。用户明确启用的驱动、CAB、XP 注入、NVMe fallback 服务注册和 Win7 CAB 预处理失败必须返回错误并停止安装；临时解包目录必须使用碰撞安全目录，注册表 hive 中途失败也必须尝试卸载并保留写入与卸载错误上下文。
