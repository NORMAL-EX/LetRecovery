...
- 扩展玻璃客户区中的单行 Edit 必须保留原生文本、光标、选择、IME 与无障碍语义；为避免 DWM 把浅色主题的黑字当作玻璃键，允许仅该子控件使用 `WS_EX_LAYERED` 与 `SetLayeredWindowAttributes(alpha=255)` 的完全不透明重定向，严禁颜色键、全局半透明或把分层样式应用到顶层窗口和导航按钮。Edit 的 `WM_CTLCOLOREDIT` 文字色必须直接使用与按钮相同的当前 `palette.text`，背景使用已经解析的字段色；退出材质模式时同步移除子控件分层样式。圆角仍由统一 8×8 覆盖率外框绘制，禁止用二值 HRGN；不透明 Edit 的外角只可与当前 Windows 11 Mica 中性回退色预混合，不能写成黑色玻璃键。非重定向 Edit 的 `HBRUSH` 仍使用目标合成色减去 Mica 中性基底后的 RGB 贡献量，禁止把已合成字段色再次叠加而使深色字段从 `#3C4660` 变亮到 `#5C6680`。ComboBox、ListBox 和 ListView 的不透明 BGRA/主题颜色路径不得套用这项画刷补偿。闭合 ComboBox 的正文、箭头区和圆角外框必须共享同一次 normal/hot/dropped 表面状态，禁止外框仍用常态色与展开态正文混合而在圆角内沿产生亮白弧线；悬停、聚焦或展开时外框必须同步使用当前明暗主题的强调色描边，不能继续使用浅灰常态描边在浅蓝交互表面上形成肉眼可见的白弧。
- 分层子 Edit 使用的是系统保存的离屏图像，不能假设父窗口显示或主题切换后 Windows 会主动再次请求绘制。主窗口完成首次 `ShowWindow`、语言切换或明暗主题的父级重绘事务后，必须枚举真实单行分层 Edit，重新确认 alpha=255，并对每个控件同步执行包含 `RDW_ERASE|RDW_FRAME|RDW_UPDATENOW` 的重绘；鼠标移动、悬停、焦点或计时器不得成为输入框首次出现或采用新主题的必要条件。
