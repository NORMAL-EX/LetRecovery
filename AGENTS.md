...
- `正常系统端/src/native_ui/tools/quick_partition.rs`：一键分区的专属 Inno 原生物理磁盘下拉、BIOS(MBR)/UEFI(GPT) 单选、已有/新规划分区明细和紧凑低圆角分区图；分区图必须是一个连续长方形，分区按容量紧贴切块、只以竖线分隔并双行显示卷标/类型与大小，不得恢复分离卡片或过高留白。只有非特殊、非当前运行 Windows 卷且右侧紧邻未分配空间的边界显示小型竖向圆角拖柄，拖动只更新暂存方案；系统卷判定必须来自当前 Windows 卷身份，禁止写死 `C:`。右键菜单必须使用确定性 owner-draw 适配明暗主题，并提供删除、可配置文件系统/卷标/分配单元/快速格式化、盘符分配/移除、MBR 活动标记、未分配空间建卷及 RAW 盘按当前固件模式初始化；已有 ESP 或无足够未分配空间时隐藏不适用的创建按钮。全部编辑由“刷新”右侧的全局“应用”一次提交，关闭存在未应用修改的窗口必须提供应用、放弃和取消选择。下拉只包含真实磁盘并保持默认空选择，库存由宿主提供，模块不枚举磁盘或执行存储写入；所有危险意图必须携带磁盘/分区指纹并由宿主二次确认。
