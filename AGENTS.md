...
- 扩展玻璃客户区中的单行 Edit 必须保留原生文本、光标、选择、IME 与无障碍语义，并始终保持为普通非分层子 HWND；禁止给 Edit 设置 `WS_EX_LAYERED`、调用 `SetLayeredWindowAttributes` 或依赖系统保存的离屏图像，因为微软明确不保证分层窗口暴露后再次请求绘制，这会造成输入框首次显示或主题切换后必须移动鼠标才出现。主题入口必须清除旧版本可能遗留的 `WS_EX_LAYERED`。Edit 的 `WM_PAINT` 必须先在 32 位顶向下 BGRA DIB 中填入最终字段色，再调用 USER32 原生 `WM_PRINTCLIENT` 绘制文字、光标和选择；GDI 不保证 Alpha 字节，必须在 `GdiFlush` 后把整帧 Alpha 修复为 255，再一次 `AlphaBlend(AC_SRC_ALPHA)` 提交，禁止用 Alpha 未定义的普通兼容位图直接 `BitBlt` 到扩展 DWM frame，否则浅色黑字会成为玻璃孔洞。`WM_CHAR`、`EM_REPLACESEL`、`WM_IME_COMPOSITION`、剪切、粘贴、清除、撤销和 Delete 等直接改变可见文字的消息必须先交给 USER32 默认处理，再在消息返回前同步重新发布最终不透明 BGRA 帧；不能只等待下一次 `WM_PAINT`，否则输入期间会短暂暴露 Alpha 未定义的白字/透明字形。DIB 创建失败时才允许直接调用原生绘制，禁止为取得浅色黑字重新引入分层重定向。圆角仍由统一 8×8 覆盖率外框绘制，外角写回父窗口玻璃键，禁止用二值 HRGN。首次 `ShowWindow`、语言切换和明暗主题事务结束时，必须在同一同步 `RedrawWindow(RDW_FRAME|RDW_ALLCHILDREN|RDW_UPDATENOW)` 中完成普通 Edit 的原生客户区与确定性外框绘制；鼠标移动、悬停、焦点或计时器不得成为输入框首次出现或采用新主题的必要条件。主窗口不得使用 `WS_EX_COMPOSITED`，否则 Edit 光标等局部更新会放大成整棵子 HWND 树的逐层中间帧。ComboBox、ListBox 和 ListView 的不透明 BGRA/主题颜色路径不得套用 Edit 画刷补偿。闭合 ComboBox 的正文、箭头区和圆角外框必须共享同一次 normal/hot/dropped 表面状态；箭头必须由带真实覆盖率的预乘 Alpha BGRA 图元发布，禁止用 Alpha 未定义的 GDI 线直接画入扩展 frame。禁止外框仍用常态色与展开态正文混合而在圆角内沿产生亮白弧线；悬停、聚焦或展开时外框必须同步使用当前明暗主题的强调色描边，不能继续使用浅灰常态描边在浅蓝交互表面上形成肉眼可见的白弧。
...
- `正常系统端/src/native_ui/backdrop.rs`：正常端顶层窗口共用的实验性 Mica 边界；关闭全窗口材质时显式写入公开 `DWMSBT_AUTO` 并撤销客户区 frame，使默认标题栏继续由 DWM 决定，启用时先确认 `DwmIsCompositionEnabled`，再设置并用 `DwmGetWindowAttribute` 回读 `DWMSBT_MAINWINDOW`，最后扩展全客户区 frame；系统不支持、回读不一致或任一步失败时必须恢复 AUTO、零 frame 和普通背景。配置请求、非 PE/DWM 支持和本顶层窗口 `WM_NCACTIVATE` 激活态必须同时成立才允许完整 Mica 会话；窗口失活必须执行与关闭实验选项完全相同的 AUTO、零 frame 和普通调色板路径，重新激活后再申请并回读 Mica。主窗口与工具对话框共用这一实现且 WinPE 不得发起 Mica 请求。
- `正常系统端/src/native_ui/redraw.rs`：主窗口和工具对话框共用的合成重绘事务；事务先记录并暂停当前可见后代，再暂停顶层根窗口，在冻结期完成显隐与布局，最后恢复仍应可见的后代并用一次同步 `RedrawWindow(...RDW_ALLCHILDREN|RDW_UPDATENOW)` 发布完整树。主窗口禁止使用 `WS_EX_COMPOSITED`，否则 Edit 光标等局部重画会扩大成可观察的整树自底向上重画；页面切换使用不擦除非客户区的轻量提交。
...
- `正常系统端/src/native_ui/theme.rs`：固定从 Inno 6.7 Modern Windows 11 明暗参考截图逐像素核对的窗口、普通/引导按钮和边框色，负责系统应用主题检测、GDI 画刷及主题资源生命周期；单行 Edit 保留原生文本、光标、选择、IME 和无障碍语义，移除宿主版本不一致的 CLIENTEDGE 后与闭合 ComboBox 共用确定性 Win11 字段底色、悬停、焦点、禁用和圆角外框，并在直接文本变更消息的默认处理后同步发布最终不透明 BGRA 客户区，不能让输入过程等待下一次库存 `WM_PAINT`；多行滚动报告继续使用 Explorer 单层边框；闭合 ComboBox、ListView 与独立 ListBox 移除会在焦点、滚动或表头悬停后复活的矩形 `WS_BORDER`/`WS_EX_CLIENTEDGE`，改由控件自身在原生绘制完成后最后覆盖确定性 5px 圆角外框，ComboBox 必须在首个 `WM_SETCURSOR`、UxTheme 悬浮 `WM_TIMER` 和焦点/按键状态的默认处理后同步补画完整闭合选区及外框，禁止先暴露矩形原生动画帧再异步覆盖；`CBS_DROPDOWNLIST` 的内部只读选择区 HWND 保留原生生命周期与无障碍语义，但必须清除 WinPE 重复库存边框并用与父 ComboBox 相同的闭合表面和文字覆盖客户区，父控件与该内部 HWND 必须视作同一焦点域，并在点击、焦点和 UI 状态的默认处理完成后同步重画内部选区和父控件完整四边外框，禁止在焦点态叠出第二条粗底线；只读的 `CBS_DROPDOWNLIST` 获得焦点后必须成对隐藏/恢复 USER32 自动创建的插入光标，避免点击瞬间出现无编辑意义的竖线；ListView 表头重绘时只在 ListView 自身坐标系补齐唯一外框，禁止把列表尺寸的圆角框画进可横向滚动的 Header DC；空表仍以当前编辑区底色完成完整绘制；非空材质 ListView 在原生绘制完成后只允许覆盖最后一条真实行以下的无项目客户区，使尾部与未选中行共用编辑区材质色，禁止重画项目行、表头、滚动条或在高频滚动消息中执行该覆盖；ComboBox 弹出选项继续由 Common Controls 6.0/UxTheme 原生绘制，不混入 `WM_DRAWITEM` 行自绘，顶层 ComboLBox 每次展开时仅重申沉浸式明暗并显式使用原生直角弹层，不叠加圆角、边框覆盖或逐行自绘；自动复选框和单选框保留 USER32 的切换/分组、键盘、焦点、通知和无障碍语义，视觉层使用从固定 Windows 11 `Aero.msstyles` 经 UxTheme 提取并嵌入的明暗、96/120/144/192 DPI 全状态图元；提取图元必须恢复直通 alpha，并用高采样圆角覆盖率裁除小数 DPI 下主题合成产生的四角暗像素，最终尺寸匹配时直接逐像素输出，禁止再次插值造成黑角或颗粒；两类控件的标题由当前明暗调色板绘制为微软雅黑，禁止深色回退成黑色标题；带 `LVS_EX_CHECKBOXES` 的 ListView 在原生绘制完成后清除状态图像槽并复用同一套 13px 复选框图元，按真实行选中底色合成且不改变 one-based 状态图像语义；`SetWindowTheme` 自己触发 `WM_THEMECHANGED`，主题入口不得再同步重复发送或在样式位未变化时强制 `SWP_FRAMECHANGED`，避免多余重绘和页面闪动；窗口激活变化只更新已经安装的子类调色板引用及 ListView 颜色，不重新应用 UxTheme 类或重建子控件几何；工具进度条采用确定性明暗绘制，无损扩大 C 盘滑块用兼容位图离屏完成背景、抗锯齿圆角轨道、绿色已选段和滑块后一次性发布，拖动、键盘和捕获变化只排队重绘当前控件且禁止擦除背景，只有设置位置/范围的 `TBM_*` 消息会失效控件，绘制期的 `TBM_GET*` 查询严禁递归触发同步重绘；ListView 的未选中行继续由 comctl32 原生绘制，仅在 Windows 11 v6 主题会覆盖 `clrTextBk` 的真实选中行通过官方 `NM_CUSTOMDRAW` 最小接管背景与各列文字，固定复用左侧导航按钮的静止选中配色，同时保留原生选择状态、列布局、滚动、键盘和无障碍语义，点击、悬停和键盘选择不得额外整表重绘；ComboBox 和传统 ListBox 不与该 ListView 选中色逻辑耦合，继续保持各自既有行为；无权限开发构建可用环境变量仅做明暗视觉回归。
  材质模式下真实单行 Edit 子 HWND 不得保留 `WS_EX_LAYERED` 或颜色键；其 `WM_PAINT` 把原生 `WM_PRINTCLIENT` 输出和材质贡献色写入同一兼容位图并一次 `BitBlt`，避免首次显示或重新暴露时先出现空字段，同时保留 USER32 文本、选择、光标、IME 与无障碍。不得把已经按中性基底解析过的实体字段色再次交给 DWM 合成，否则会产生双重混色。
