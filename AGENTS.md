...
- `正常系统端/src/native_ui/dialog.rs`：Inno 风格原生模态/非模态对话框壳、主动按当前明暗主题绘制且把内容容器收到的命令、通知、颜色及 `WM_HSCROLL` 原样转发给所有者、ComboBox `WM_DRAWITEM` 和 ListView 表头的内容容器、按 DFM 约 46px 基线收紧的命令栏、工具弹窗同级命令按钮保持 10px 逻辑间距、以 75px 为下限并按实际微软雅黑 UI 字体/DPI 测量译文扩宽的命令按钮、仅响应 `BN_CLICKED` 且完整抑制擦除白闪的稳定自绘按钮、已有非模态窗口激活、按显示器工作区约束的高 DPI 尺寸、长说明实测换行与内容自然高度收紧、工作区钳制和消息转发；首次显示前统一为遗漏字体的后创建子控件补充微软雅黑并预先应用 Edit/ComboBox/ListBox/ListView/按钮明暗主题，顶层对话框保持普通不透明命中测试并在显示后同步发布完整首帧，不得使用分层 alpha 暂存，工具弹窗专用窗口类的 `hIcon/hIconSm` 固定为空且“关闭”操作只保留标题栏 X（取消操作仍保留命令按钮），主窗口独立类继续使用发布图标；异步结果不得清除尚未消费的关闭/命令结果或复活已隐藏窗口，不承载业务副作用。
...
- 单选框绘制补充约束（覆盖上一条中“复选框和单选框都使用嵌入图元”的旧描述）：单选框禁止使用已与主题表面预合成且只有二值 alpha 的捕获 PNG，必须按当前物理像素尺寸以固定 Windows 11 状态色和至少 8×8 子像素覆盖率生成完全不透明、四向对称的圆环或选中圆点，禁止 GDI 再插值造成浅色黑点或颗粒；USER32 仍独占分组、切换、键盘、通知和无障碍语义。
- 正常端实验性窗口背景约束：仅主窗口按 `config.json` 的 `experimental_window_backdrop` 请求公开的 `DWMWA_SYSTEMBACKDROP_TYPE`，映射 `auto/mica/acrylic/mica_alt` 到系统 Auto/MainWindow/TransientWindow/TabbedWindow。显式 `mica/acrylic/mica_alt` 在明暗主题下都可使用负边距全客户区 DWM frame：基础层和圆角外部像素使用黑色玻璃键，交互表面使用克制的材质专用实体色；浅色深色字形、透明 STATIC、复选/单选标题、按钮、ComboBox、Header、ListView 行和空表必须分别走经过实机验证的 Alpha 安全路径，不得用普通 GDI 兼容位图、近黑色或主题发光文字冒充正确合成。控件子类引用必须同时保存明暗与材质状态，材质接受状态确定后必须同步重建字段画刷，空 ListView 的 `WM_PAINT` 必须填满完整客户区而不能仅依赖可能为空的 `rcPaint`。`none`、不支持构建或任一 DWM 调用失败回退普通背景；工具对话框和 PE 端不得继承。实现及微软来源见 `docs/EXPERIMENTAL_WINDOW_BACKDROPS.md`。
- `正常系统端/src/native_ui/window.rs`：正常端原生窗口类、显式大/小应用图标、消息循环、Per-Monitor V2 DPI、统一 Microsoft YaHei UI 字体、受当前显示器工作区约束的 DPI 最小跟踪尺寸、国际化导航/页头/命令栏和页面布局；主窗口统一转发 ComboBox owner-draw，ListView 表头只由 `theme.rs` 的唯一入口绘制；页面切换批量暂停重绘并在显隐和布局稳定后一次性重绘全部子控件，避免暴露中间布局造成闪烁；安装页对驱动、重启、引导模式及无人值守控件做响应式分行，宽窗口中立即重启按实测译文宽度紧跟驱动下拉保持标准字段间距，取消无人值守后不再保留隐藏的文件选择和提示槽位，而是按当前译文宽度把可容纳的高级操作即时重排到引导模式之后；启用高级选项时 DiskPart 复选框、打开目录和修改引导命令按实测微软雅黑宽度紧凑排列并与条件启动签名选择保持正常组间距，启动签名标签与下拉框只保留标准字段间距；底部命令栏保留稳定内边距，硬件页隐藏刷新后把保存和复制从右向左紧密排列，复制成功只局部刷新按钮为本地化“已复制”并用非阻塞三秒定时器稳定恢复，不使用缩放、弹性动画或整页重绘；分区动态状态随语言切换重新翻译。窗口统一过滤 Button 的非点击通知，工具对话框只响应一次真实点击且已有窗口优先激活，避免焦点通知导致重复窗口和后台任务；GHO、镜像和哈希浏览内容命令直接打开文件选择器并回填原对话框，不借用会隐藏窗口的命令栏结果；工具对话框关闭结果先于异步完成消息处理，轮询存活覆盖全部专用窗口和后台任务，关闭 A 后打开 B 不得复活 A；分区对拷首次库存仍在后台只读枚举，完成后额外投递窗口唤醒消息立即消费结果，定时轮询仅作为失败安全回退，不要求用户点击刷新；系统目标在桌面默认当前系统、PE 默认首个离线系统，并按规范化 `SystemDrive` 排除与在线系统重复的离线盘符项；物理磁盘和普通数据卷等不可逆目标仍不得自动预选。窗口把原生页面意图路由到安装、备份、异步下载目录及工具控制器、带代次校验的异步只读库存和强类型后台请求边界（包括区分在线/离线目标的 APPX 请求）；在线目录刷新后以同一 PE 快照驱动下拉显示和执行映射，单一 PE 自动选中但不显示无意义选择器，安装 PE 下拉使用明暗字段主题；安装或备份在首次点击、BitLocker 解锁和 PE 下载续做后都重新枚举并复核稳定分区身份、路由及锁定状态，简易模式下载完成后重新读取真实镜像卷和架构；安装页同时保留源/目标无人值守探测、多 PE、与引导修复和实际 UEFI 条件一致的 PCA2011/PCA2023 检测与选择，自动启动签名项必须显示当前实际推荐代次，以代次及目标磁盘/分区身份隔离异步同盘 ESP 只读检测结果，并仅在高级 DiskPart 明确可能创建 ESP 时按既有规则放行缺失状态；已退役 Windows 7 UefiSeven/USB3/NVMe 资源对应的高级选项始终隐藏并在镜像切换时清零，存储驱动工具必须按当前 SetupAPI PCI ID 选出唯一 Intel VMD 子目录且禁止递归；实验性背景在 DWM 确认后按实际材质状态同步重建全部主窗字段画刷，明暗显式材质都启用负边距全客户区 frame 并在完整子控件树应用 Alpha 合成主题，失败路径必须撤销 frame 并回到不透明客户区；执行失败时仅展示本地化摘要而把底层诊断留在日志；安装、备份、下载、扩容或镜像校验仍在运行时拦截窗口直接关闭，先请求取消并等待安全停止点；工具页负责把专属一键分区与 BitLocker 对话框接入 fresh 库存、二次确认和后台安全边界，并把镜像校验的实时进度及取消标志贯通到既有校验器；Wi-Fi 迁移只在已连接网络时抓取本次运行所需的瞬态配置；无损扩大 C 盘使用专属分析、二次确认、PE 下载及交接状态机。
  主窗口和工具对话框不使用顶层分层首帧暂存；语言变更由 `WM_SETREDRAW` 包围全部译文、库存和布局更新，再用一次同步 `RedrawWindow` 发布，避免闪烁及混合语言帧。主窗口扩展样式不得包含 `WS_EX_LAYERED` 或 `WS_EX_TRANSPARENT`，保证导航命中不会穿透到后方窗口。
...
- `正常系统端/src/core/app_config.rs`：`config.json` 用户偏好、语言、日志、外观和默认选项的读取保存；小白模式与高级选项保持互斥并归一旧配置冲突；下载线程字段向后兼容旧配置并在加载与保存入口归一到 8/16/32 三档，旧配置保持原有 16 连接默认值；正常端实验性 `experimental_window_backdrop` 只接受 `none/auto/mica/acrylic/mica_alt`，旧配置和未知值安全回退 `none`。
