...
- `lr-core/src/wimlib.rs`：动态封装 `libwim-15.dll` 的打开、校验、释放、捕获、拆分和进度回调；打开 WIM 后优先启用自定义 DLL 的有界并行解压与并行校验扩展，显式把当前可提交内存的四分之一（最高 2 GiB）作为并行工作预算，旧 DLL 缺少可选导出时保持串行兼容；自定义 Win32 应用路径只在硬链接结构和全部 inode 元数据已完整成功时复用首个别名的结果，任何被忽略的结构错误、对象 ID、EA 或 ACL 部分失败都必须让后续别名重试并在所有退出路径清理临时标记；镜像应用与校验接受调用方持有的原子取消标记，并由 libwim 进度回调返回中止状态以真正停止当前操作。
...
- `正常系统端/src/native_ui/scrollbar_compositor.rs`：高级选项页滚动条的 Composition/GDI 双缓冲发布边界；负责帧位图所有权、合并重绘、窗口定位和透明命中测试，主题图元状态、滚动模型和无障碍语义仍由 `pages/advanced.rs` 与原生 `WS_VSCROLL` 提供。
...
- `正常系统端/src/core/native_install_backend.rs`：原生安装执行器的生产后端阶段实现；复用既有镜像、格式化、驱动、引导和高级选项边界，原版 XP/2003 当前系统盘重装会递归拒绝链接和特殊项、统计空间、把 I386/AMD64 及 x64 同级 I386 复制到会话唯一临时目录、复验后原子提交给 PE；ViaPE 安装源若与目标数据区路径相同必须按已存在源复核而非删除或复制自己，其他路径必须复制到会话唯一暂存文件，在复制、落盘、可取消复读哈希和镜像结构校验期间连续汇报进度，验证完成后再原子发布且不做无价值的第三次发布后哈希。单文件 WIM/ESD 在拒绝源写入和替换的句柄锁内并行执行完整校验与复制/源 SHA-256，目标同步落盘并复读 SHA-256 完全相等后继承源校验结果；SWM、GHO、ISO 和 XP 路径继续使用原有独立校验语义。Direct 安装的 Windows 10/11 默认无人值守在写入前必须通过 `lr-core::offline_international` 读取已释放目标的国际化设置，不能从当前正常系统或 WinPE 宿主继承；显式保留 Wi-Fi 瞬态 profile，恢复后台 fresh BitLocker 门禁、无恢复密钥时的完全解密等待、DiskPart 后按稳定分区身份重定位/分配盘符，以及 WIM/GHO/校验长任务的实时进度与安全取消反馈；WIM/ESD/SWM 应用和校验贯穿调用方原子取消标记，libwim 可中止当前操作，取消后禁止引擎回退和后续安装阶段；用户请求的主机驱动导出和后续导入必须验证非零结果并失败关闭，导入或保存失败时保留暂存备份，开发测试构建仍保持无副作用。
