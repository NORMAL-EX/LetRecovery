...
- 正常端主窗口和全部工具对话框在首次显示前必须完成字体、主题、布局和后创建子控件准备；首次显示必须先以非零全局 alpha 的近透明分层顶层窗口完成整棵原生控件树的同步首帧绘制，再一次性恢复不透明并立即清除 `WS_EX_LAYERED`，随后用 `SWP_FRAMECHANGED` 刷新命中测试与非客户区缓存；严禁使用 alpha=0，因为 Windows 会让零 alpha 分层像素的鼠标输入穿透到下层窗口。分层状态不得进入导航、页面切换或后续布局重绘，精简版 WinPE 拒绝分层属性时才允许退回无库存背景擦除的同步重绘路径；显示调用返回消息循环或异步库存启动前，必须完整绘制非客户区、客户区及所有子控件，禁止让 WinPE 先呈现白色 Edit、ComboBox、ListView 或按钮中间帧后再靠定时器、鼠标悬停修复。

- 扩展玻璃客户区中的 Edit `HBRUSH` 必须使用目标合成色减去 Mica 中性基底后的 RGB 贡献量，禁止把已合成字段色再次叠加而使深色字段从 `#3C4660` 变亮到 `#5C6680`；ComboBox、ListBox 和 ListView 的不透明 BGRA/主题颜色路径不得套用这项画刷补偿。闭合 ComboBox 的正文、箭头区和圆角外框必须共享同一次 normal/hot/dropped 表面状态，禁止外框仍用常态色与展开态正文混合而在圆角内沿产生亮白弧线；悬停、聚焦或展开时外框必须同步使用当前明暗主题的强调色描边，不能继续使用浅灰常态描边在浅蓝交互表面上形成肉眼可见的白弧。
...
- `正常系统端/src/native_ui/dialog.rs`：Inno 风格原生模态/非模态对话框壳、主动按当前明暗主题绘制且把内容容器收到的命令、通知、颜色及 `WM_HSCROLL` 原样转发给所有者、ComboBox `WM_DRAWITEM` 和 ListView 表头的内容容器、按 DFM 约 46px 基线收紧的命令栏、工具弹窗同级命令按钮保持 10px 逻辑间距、以 75px 为下限并按实际微软雅黑 UI 字体/DPI 测量译文扩宽的命令按钮、仅响应 `BN_CLICKED` 且完整抑制擦除白闪的稳定自绘按钮、已有非模态窗口激活、按显示器工作区约束的高 DPI 尺寸、长说明实测换行与内容自然高度收紧、工作区钳制和消息转发；首次显示前统一为遗漏字体的后创建子控件补充微软雅黑并预先应用 Edit/ComboBox/ListBox/ListView/按钮明暗主题，首帧透明暂存完成后立即清除 `WS_EX_LAYERED`，不得让分层窗口状态进入后续布局和重绘，工具弹窗专用窗口类的 `hIcon/hIconSm` 固定为空且“关闭”操作只保留标题栏 X（取消操作仍保留命令按钮），主窗口独立类继续使用发布图标；异步结果不得清除尚未消费的关闭/命令结果或复活已隐藏窗口，不承载业务副作用。
