...
- `.github/workflows/`：PR、主分支和发布流水线；release 在替换 PE 原生 Win32 EXE 时必须同时从底包 WIM 删除旧 `opengl32.dll`，不得把已停用的 egui/glow 运行时重新带入发布包；正式或预发布 Release 必须在最终 `pkg/` 生成后使用固定 Inno Setup 6.7.1 构建离线安装器，完整校验后只上传 `LetRecovery.7z` 和 `LetRecovery-Setup-x64.exe` 两个资产。
