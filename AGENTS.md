...
- 扩展玻璃客户区中的单行 Edit 必须保留原生文本、光标、选择、IME 与无障碍语义，并始终保持为普通非分层子 HWND；禁止给 Edit 设置 `WS_EX_LAYERED`、调用 `SetLayeredWindowAttributes` 或依赖系统保存的离屏图像，因为微软明确不保证分层窗口暴露后再次请求绘制，这会造成输入框首次显示或主题切换后必须移动鼠标才出现。主题入口必须清除旧版本可能遗留的 `WS_EX_LAYERED`。Edit 的 `WM_PAINT` 必须通过顶向下 `BeginBufferedPaint` 缓冲调用 USER32 原生 `WM_PRINTCLIENT`，在 `GdiFlush` 后用 `BufferedPaintSetAlpha(..., 255)` 把文字、选择和字段背景作为一个明确不透明结果提交到玻璃；缓冲事务中的 `WM_CTLCOLOREDIT` 使用已解析字段色和与按钮相同的精确 `palette.text`，不得让浅色黑字再次成为 DWM 玻璃键。缓冲初始化失败时才允许回退到普通 GDI 贡献色，禁止为取得黑字重新引入分层重定向。圆角仍由统一 8×8 覆盖率外框绘制，外角写回父窗口玻璃键，禁止用二值 HRGN。首次 `ShowWindow`、语言切换和明暗主题事务结束时，必须在同一同步 `RedrawWindow(RDW_FRAME|RDW_ALLCHILDREN|RDW_UPDATENOW)` 中完成普通 Edit 的原生客户区与确定性外框绘制；鼠标移动、悬停、焦点或计时器不得成为输入框首次出现或采用新主题的必要条件。ComboBox、ListBox 和 ListView 的不透明 BGRA/主题颜色路径不得套用 Edit 画刷补偿。闭合 ComboBox 的正文、箭头区和圆角外框必须共享同一次 normal/hot/dropped 表面状态，禁止外框仍用常态色与展开态正文混合而在圆角内沿产生亮白弧线；悬停、聚焦或展开时外框必须同步使用当前明暗主题的强调色描边，不能继续使用浅灰常态描边在浅蓝交互表面上形成肉眼可见的白弧。
...
- `正常系统端/src/native_ui/backdrop.rs`：正常端顶层窗口共用的实验性 Mica 开关，只用公开 `DWMWA_SYSTEMBACKDROP_TYPE=DWMSBT_MAINWINDOW` 与全客户区 frame；关闭、系统不支持或任一步失败时撤销 frame 并回退普通背景，主窗口与工具对话框必须共用这一实现。
- `正常系统端/src/native_ui/redraw.rs`：主窗口和工具对话框共用的合成重绘事务；主窗口以 `WS_EX_COMPOSITED` 提供后代自底向上的双缓冲，因此事务只暂停可见顶层根窗口，完成显隐与布局后再用一次同步 `RedrawWindow(...RDW_ALLCHILDREN|RDW_UPDATENOW)` 发布完整树，页面切换使用不擦除非客户区的轻量提交，禁止逐个暂停和恢复大量子 HWND。
- `正常系统端/src/native_ui/controls.rs`：原生子控件创建、UTF-16 字符串、库存下拉框空选择哨兵与无偏移索引校验、按 Inno DFM 的 23px 高/75px 最小宽基线缩放的 DPI 尺寸，以及按钮、分隔线和进度条的明暗/悬停/按下/禁用/单焦点边框状态绘制；ComboBox 保持 Common Controls 6.0 的库存字符串、键盘、无障碍和原生弹层语义，不得全局强加 `CBS_OWNERDRAWFIXED` 或在每个弹出项重绘时同步读取文本和分配缓冲；所有自绘按钮在共用子类中用 `TrackMouseEvent(TME_LEAVE)` 跟踪悬停，只局部重绘自身并复用 normal/hot/pressed/disabled 调色板，跟踪失败、取消模式、隐藏复用和禁用时立即清除热态，不因焦点增加双边框；按钮、长任务进度条及闭合字段/列表外框使用限域抗锯齿绘制，圆弧厚度必须随 DPI 与直边同步，重复重绘使用绝对调色板颜色以避免颗粒、断线和逐帧变暗；全客户区 DWM 材质下，自绘按钮与圆角表面必须通过 32 位顶向下 BGRA 缓冲保留真实覆盖率 Alpha，同尺寸/半径/调色板图元使用有界线程本地缓存复用，禁止页面切换时重复构造相同 4 倍圆角位图；实体控件文字使用白色 GDI 字形遮罩重着色为预乘 Alpha；透明 STATIC 的顶层直接子项继续用 UxTheme 玻璃文字缓冲避免出现实体标签矩形，而工具内容容器中的嵌套 STATIC 必须改用确定性白色 GDI 遮罩和预乘 Alpha，因为嵌套 HWND 会丢失 UxTheme 贡献的文字 Alpha；两条路径必须得到相同的明暗文字色且都不得产生发光标题或不透明标签条；CPU 读取或写入经过 GDI 绘制的 DIB bits 前必须调用 `GdiFlush`，禁止让延迟 GDI 批处理与像素访问交错；单行编辑框从创建期保留 `WS_CHILD|WS_VISIBLE|WS_TABSTOP|ES_AUTOHSCROLL` 和 `WS_EX_NOPARENTNOTIFY`，移除宿主版本会复活的 `WS_BORDER`/`WS_EX_CLIENTEDGE`，文本、光标、选择、IME 与无障碍仍完全由原生 Edit 负责，外层闭合字段视觉与 ComboBox 共享确定性 Win11 圆角框；统一布局子类只调整真实单行 Edit 的 HWND 高度，使其按 23px 字段基线在响应式行内上下居中，直接创建的安装镜像和高级选项字段也必须接入，不得用 `EM_SETRECT` 或自绘文字伪造居中；进度条保持深色轨道、统一 Inno 绿色与 5px 内克制圆角。
...
- `正常系统端/src/native_ui/dialog.rs`：Inno 风格原生模态/非模态对话框壳、主动按当前明暗主题绘制且把内容容器收到的命令、通知、颜色及 `WM_HSCROLL` 原样转发给所有者、ComboBox `WM_DRAWITEM` 和 ListView 表头的内容容器、按 DFM 约 46px 基线收紧的命令栏、工具弹窗同级命令按钮保持 10px 逻辑间距、以 75px 为下限并按实际微软雅黑 UI 字体/DPI 测量译文扩宽的命令按钮、仅响应 `BN_CLICKED` 且完整抑制擦除白闪的稳定自绘按钮、已有非模态窗口激活、按显示器工作区约束的高 DPI 尺寸、长说明实测换行与内容自然高度收紧、工作区钳制和消息转发；首次显示前统一为遗漏字体的后创建子控件补充微软雅黑并预先应用 Edit/ComboBox/ListBox/ListView/按钮明暗主题，正常系统端启用实验性 Mica 时所有工具对话框必须继承同一全客户区 Mica 和 Alpha 安全子控件主题，主题变化通过共享整树重绘事务一次发布；顶层对话框仍保持普通不透明命中测试且不得使用分层 alpha 暂存，工具弹窗专用窗口类的 `hIcon/hIconSm` 固定为空且“关闭”操作只保留标题栏 X（取消操作仍保留命令按钮），主窗口独立类继续使用发布图标；异步结果不得清除尚未消费的关闭/命令结果或复活已隐藏窗口，不承载业务副作用。
