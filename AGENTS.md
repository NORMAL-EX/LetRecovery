...
cargo clippy --workspace --all-targets --locked --features "LetRecovery/non-elevated-tests,letrecovery-pe/non-elevated-tests" -- -D warnings
...
PCA2023 离线资源必须使用 `.github/scripts/build-pca2023-pack.ps1` 从已维护的官方 `boot.wim` 制作，并按 `docs/PCA2023_COMPAT_PACKAGES.md` 记录来源和虚拟机矩阵。正式 release 会检查三份资源并把它们同时放入桌面端和 PE WIM；不得为了让流水线通过而使用空 WIM、某台机器的 ESP 备份或未经验证的 Insider 文件。
