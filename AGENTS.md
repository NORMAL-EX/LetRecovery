...
- 内置 Administrator 高级选项只允许在 Windows 7 及以上 WIM/ESD/SWM 的内置无人值守路径启用；不得与自定义无人值守文件、普通“自定义用户名”、GHO/GHS 或 XP/2003 路径并用。“自定义用户名”和“启用内置 Administrator”必须作为同组原生单选框互斥，普通自定义用户名为默认项；用户名为空并保存高级选项时按当前登录用户名、经安全裁剪的系统厂商名、`User` 顺序补齐，补齐值只保留在当前安装会话且 `config.json` 必须跳过 `username`，重新打开高级选项仍应显示本会话值。内置 Administrator 账户名加载或保存时为空必须恢复为 `Administrator`，不得让空账户名进入安装意图；自定义系统盘卷标的输入默认值为 `OS`。账户改名必须按 RID-500 身份定位，不能依赖本地化账户显示名；密码只允许存在于当前安装会话配置和 Windows Setup 无人值守文件中，持久化用户偏好、调试输出和日志必须脱敏，安装清理仍需删除会话文件。
...
- 正常端高级选项页保留拥有 `WS_VSCROLL` 的原 viewport 作为唯一滚动范围、键盘和无障碍语义来源，但该 HWND 的窗口区域必须只包含矩形内容区，彻底裁掉库存非客户区滚动槽及其外圈；`SetScrollInfo` 只更新模型并使用 `redraw=FALSE`，禁止让 USER32 与最终可见滚动条竞争重定向表面。最终可见滚动条由同父、无 ID 的覆盖 STATIC 承载并与最右字段保持 24 个逻辑像素的独立间距，使用实时页面底色和固定 Windows 11 `aero.msstyles`（浅色）/`Dark.msstyles`（深色）中 `ScrollBar/ARROWBTN`、`UPPERTRACKVERT`、`LOWERTRACKVERT` 与 `THUMBBTNVERT` 的原始图元；鼠标命中和拖动必须按 `GetScrollBarInfo` 的真实部件几何及 `GetScrollInfo` 的范围换算后送回既有 `WM_VSCROLL` 状态机，禁止直接移动页面或另建第二套偏移。96/120/144/192/240 DPI 和 NORMAL/HOT/PRESSED/DISABLED/HOVER 五态由构建脚本从状态条确定性切片，按各部件自带 `IMAGESELECTTYPE=DPI`、`SIZINGTYPE=STRETCH` 和 `SIZINGMARGINS` 规则选择与九宫拉伸。`ARROWBTN` 和 `THUMBBTNVERT` 的 `TMT_TRANSPARENT` 色键背景必须在生成 BGRA 表时转换为 alpha=0；`UPPERTRACKVERT`/`LOWERTRACKVERT` 虽为不透明主题资源，也必须在生成期仅把与每帧左上角完全相同的 `#202020`/白色宿主表面转换为透明，保留真实轨道像素，禁止把宿主表面画到颜色不同的页面上形成贯穿全高的外圈。未悬浮时只能绘制 `THUMBBTNVERT` 的细线 Normal 帧，轨道和箭头必须完全隐藏；进入任一部件时必须先连续绘制上下轨道，再绘制箭头和滑块，命中部件使用 Hot、其余可见图元使用 Hover；按下只把命中部件改为 Pressed，禁用时轨道和箭头隐藏且滑块使用 Disabled。不得以任意灰度、通用圆角矩形或宿主机版本主题冒充提取资源。页面底部状态槽的真实右边界必须避开居中的“保存并返回”按钮，不能继续按右对齐命令组的缓存边界扩张后用不透明 STATIC 覆盖按钮正文。
...
- 正常端高级选项页的滚轮输入必须合并为目标偏移并按约 16ms 帧节奏平滑收敛；实际滚动应优先用一次 `ScrollWindowEx(SW_SCROLLCHILDREN|SW_INVALIDATE)` 移动同父子控件且不得附带 `SW_ERASE`，只在精简 USER32 拒绝时回退完整布局，禁止每帧逐控件执行可见 `MoveWindow`。会被 `ScrollWindowEx` 复制的直接子级 STATIC 必须在重绘时用页面画刷不透明擦除文字背景，滚动后至少显式无擦除失效可能独立重绘的条件标签，禁止透明文字叠在已复制字形上形成一帧偏移的重影。内置 Administrator 的账户名和密码标签不得通过 `EnableWindow(FALSE)` 进入 USER32 禁用 STATIC 的浮雕双层文字路径；两者保持启用并使用 `SS_OWNERDRAW`，每帧先完整擦除页面底色，再根据关联 Edit 的启用状态只绘制一次正常或禁用文字。提取资源滚动条禁止使用 `GetWindowDC` 即时画屏，状态、拖动位置和重画请求必须合并为队列中最多一个自定义帧消息，最新位置覆盖旧位置；正常系统优先由运行时动态加载的 D3D11（硬件失败后 WARP）、Direct2D 与 DirectComposition 把完整不透明 BGRA 帧作为一次 `Commit` 发布，设备丢失、DLL 缺失或任何 HRESULT 失败时记录阶段和错误并永久切到唯一 `WM_PAINT` 回退。回退只能在 `BeginPaint/EndPaint` 中把背景、上下轨道、箭头和滑块先合成到兼容内存 DC，再用单次 `BitBlt` 发布，禁止恢复同步屏幕 DC、双重画家或每条 `WM_MOUSEMOVE` 一次提交。`WM_NCMOUSEMOVE` 必须合并相同热态，库存 `WM_TIMER` 不得触发同步整控件重画。
...
- 正常端复制安装源到 PE 数据分区时，复制、落盘、复读 SHA-256、镜像结构校验和原子发布必须位于同一条 0–100% 进度带内，并在复读与结构校验期间持续响应取消；不能在复制结束后先显示 100%，再无反馈地执行长时间同步或重复哈希。正常端安装任务运行期间左下角状态槽保持为空，仅在取消、失败或终态需要明确提示时显示文案。
...
- `lr-core/src/unattend_account.rs`：内置 RID-500 Administrator 高级选项、敏感字符串脱敏与非持久化边界、账户名/密码验证，以及两端复用的 AdministratorPassword、AutoLogon 和安全编码改名无人值守片段生成。
...
- `正常系统端/src/native_ui/scrollbar_compositor.rs`：高级选项滚动条的可选 DirectComposition 发布边界；仅从系统目录运行时解析 D3D11、Direct2D 和 DirectComposition，创建 BGRA 设备时硬件失败回退 WARP，以 `IDXGISurface` 建立 Direct2D 目标并把调用方提供的完整不透明主题帧作为单次合成事务提交。模块不得拥有滚动范围、输入或页面偏移；DLL/设备/表面/绘制/提交任一阶段失败必须记录具体阶段与 HRESULT，并让调用方切换到 `WM_PAINT`，精简 WinPE 缺少图形 DLL 时不得形成加载期依赖或阻止程序启动。
...
- `正常系统端/src/native_ui/pages/advanced.rs`：安装高级选项的原生分组控件、条件显示、运行期完整本地化、依赖启用、Wi-Fi SSID 状态和 `AdvancedOptionsData` 双向映射；内置 Administrator 账户名、掩码密码和自动登录控件只在兼容的内置无人值守路径可用，并与普通自定义用户名互斥；响应式列宽必须预留 DPI 缩放后的垂直滚动条宽度，禁止最后一列 Edit 或浏览按钮被滚动条及窗口右边缘裁切；本文件同时维护高级页原生滚动范围、平滑滚动、固定主题资源滚动条覆盖层及其鼠标代理，覆盖层只改变最终可见图元和指针路由，键盘、无障碍和页面偏移仍复用原 viewport 与 `WM_VSCROLL` 状态机；复选框和单选框必须接入 `theme.rs` 的确定性明暗绘制器，使用与主安装页一致的 24px HWND 高度，并在页面最终显示后重申当前主题，运行期主题切换后不得保留旧的黑色标题或形成不同的图元对齐；条件 Edit/Browse 的复选框点击由窗口控制器转发后立即更新启用态，Edit 和浏览按钮必须使用共享 `InnoMetrics` 的 23px 字段高度，使原生单行 Edit 文本与其他页面相同地上下居中，禁用 Edit 仍保持与普通字段一致的底色和边框；为脚本、驱动和自定义内容路径生成明确的浏览意图并接收窗口控制器回填，返回时对“已勾选但必填文本为空”的配对选项同步取消并持久化关闭，不自行打开文件对话框或执行离线系统写入。
...
- `正常系统端/src/core/advanced_options.rs`：安装高级选项的序列化兼容、离线系统应用、脚本/文件暂存、Wi-Fi 瞬态配置、内置 Administrator 非持久化密码选项和历史 XP/Win7 兼容业务边界；不再包含 egui 或其他渲染代码，原生界面映射由 `native_ui/pages/advanced.rs` 负责。
...
- `正常系统端/src/core/install_config.rs`：正常端到 PE 的安装、备份、扩容配置，安装标记、资源暂存和无人值守文件验证；当前安装会话可暂存内置 Administrator 账户名、密码与自动登录选择，但密码不得进入持久化应用配置或日志；XP 文本模式交接显式记录目录型源标记和 I386/AMD64 安全子目录；扫描覆盖 A-Z 并用 `SessionId` 唯一绑定安装标记与配置，旧空会话配置仅在单一无歧义任务时兼容，所有写入 INI 的值在落盘前拒绝 CR/LF/NUL 注入；备份与扩容交接配置使用同目录原子替换并返回可精确恢复旧文件或仅删除本次新文件的事务凭据，不删除数据目录中的其他用户文件；回滚会独立尝试恢复配置与标记并合并错误，避免单项失败阻断另一项恢复。
...
- `正常系统端/src/core/native_install_executor.rs`：原生安装意图的 Direct/ViaPE 有序阶段计划、BitLocker/PCA/稳定目标安全门、Direct 驱动导出先于目标 DiskPart/格式化的保护顺序、按实际耗时把镜像应用和源复制置于主要区间的加权总进度、进度取消消息和可注入副作用后端；`InstallExecutionError::user_message` 按失败类别和阶段提供不泄露底层诊断细节的本地化用户摘要，源镜像校验失败还必须展示不含路径的常见原因与固定诊断代码，使单张截图可供支持判断，完整 code/detail 继续保留在 `Display` 日志边界，开发测试构建强制拒绝执行。
...
- `正常系统端/src/core/native_install_backend.rs`：原生安装执行器的生产后端阶段实现；复用既有镜像、格式化、驱动、引导和高级选项边界，原版 XP/2003 当前系统盘重装会递归拒绝链接和特殊项、统计空间、把 I386/AMD64 及 x64 同级 I386 复制到会话唯一临时目录、复验后原子提交给 PE；ViaPE 安装源若与目标数据区路径相同必须按已存在源复核而非删除或复制自己，其他路径必须复制到会话唯一暂存文件，在复制、落盘、可取消复读哈希和镜像结构校验期间连续汇报进度，验证完成后再原子发布且不做无价值的第三次发布后哈希。Direct 安装的 Windows 10/11 默认无人值守在写入前必须通过 `lr-core::offline_international` 读取已释放目标的国际化设置，不能从当前正常系统或 WinPE 宿主继承；显式保留 Wi-Fi 瞬态 profile，恢复后台 fresh BitLocker 门禁、无恢复密钥时的完全解密等待、DiskPart 后按稳定分区身份重定位/分配盘符，以及 WIM/GHO/校验长任务的实时进度与安全取消反馈；WIM/ESD/SWM 应用和校验贯穿调用方原子取消标记，libwim 可中止当前操作，取消后禁止引擎回退和后续安装阶段；用户请求的主机驱动导出和后续导入必须验证非零结果并失败关闭，导入或保存失败时保留暂存备份，开发测试构建仍保持无副作用。
- `正常系统端/src/core/native_install_compat.rs`：Direct 安装的默认无人值守 XML、版本化用户驱动目录、MBR 零签名/XP active 清理及 DiskPart→format.com 回退的无副作用兼容计划和解析；内置 Administrator 无人值守片段必须复用 `lr-core::unattend_account`；Windows 10/11 渲染必须显式接收已验证国际化设置并完整写入 `Microsoft-Windows-International-Core` 与时区，缺失时失败关闭，不得恢复只对 Server 生效的 `HideLocalAccountScreen`。
...
- `PE端/src/native_ui/progress.rs`：PE P3/P4/P5 安装、备份和无损扩容的原生 Win32 长任务页，复用共享 `WorkflowSession` 单次启动 worker，以 50ms 定时器合并既有消息并只更新变化的文字/进度区域；加载圈另由约 16ms 的高精度 waitable timer 独立生产帧（不支持时回退常规 waitable timer 或线程时钟），通过合并的 `WM_APP` 消息保证队列内最多一个待处理帧，只覆盖步骤列表中真实 `InProgress` 行的 16px 状态图元，首次布局尚无当前步骤时必须在后续真实步骤转换中重新绑定非空动画矩形并重置该步骤的动画起点；该父窗口自绘槽必须在每个完整不透明帧后执行 `GdiFlush` 再释放窗口 DC，避免 WinPE/VMware 省略微小父窗口更新区域或把后续帧留在 GDI 批处理中，不得放在顶部进度条旁、依赖低优先级 `WM_TIMER` 或退化为 50ms 工作轮询动画；动态安装/备份步骤保留无分配的静态键，仅在步骤列表实际重绘时本地化，英语模式不得残留中文且轮询不得为翻译反复分配；顶部以 16px 逻辑标题字体显示助手名称，不创建具体步骤名称或“当前步骤”标题，两条进度信息分别以独立 9px 逻辑字体显示“步骤进度:”和“总体进度:”，运行时按当前语言测量共同标签宽度，使标签与进度条的完整可见块水平居中；标签向进度条右对齐，百分比数字完全隐藏，进度变化只重绘胶囊条；步骤文字必须使用 `SS_CENTERIMAGE` 在 24px 逻辑行内垂直居中，使其与同样按行中心定位的 16px 状态图元对齐；完成步骤文字和徽标使用统一绿色，只有真实 `InProgress` 步骤使用当前主题主按钮同款蓝色加载圈与文字，状态切换后必须先发布新语义再同步重绘子 `STATIC`，禁止只重绘父窗口图标而遗留旧文字颜色；无损扩容的总进度直接跟随既有百分比；纯进度页不绘制步骤列表分隔线，不创建只读状态 Edit、不显示高级选项或客户区关闭按钮，也不创建“操作进行中”“正在完成清理和收尾操作”等底部状态文案或空状态栏，详细诊断写入日志；工作流运行时必须灰显标题栏系统关闭命令并继续拒绝 `WM_CLOSE`，只有终态且 worker 已安全结束后才能恢复，禁止用警示标语代替关闭边界；顶层样式必须移除 `WS_MAXIMIZEBOX`，不得显示或响应最大化按钮；默认窗口使用 480×440 逻辑尺寸和 440×430 逻辑最小尺寸，初始窗口通过当前 HWND 最近显示器的工作区居中，`WM_DPICHANGED` 建议矩形仍夹到同一工作区；窗口类不设置图标且顶层窗口使用无标题栏图标的对话框框架扩展样式；`non-elevated-tests` 的运行态预览必须复用生产加载圈、高精度动画调度、绘制路径和受限 `WorkflowSession` 消息接收路径，并注入不访问磁盘的合成进度洪峰与真实步骤转换，另提供失败状态视觉入口以验收叉号；两者均不启动 worker 或访问磁盘；消息循环异常仍等待同一 worker 真正结束；检查点、完成重启和底层错误语义保持不变。
