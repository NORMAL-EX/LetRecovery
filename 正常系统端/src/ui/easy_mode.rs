...
impl App {
            ...
            ui.vertical(|ui| {
                ...
                if is_selected {
                    ...
                    ui.vertical_centered(|ui| {
                        if !system.volume.is_empty() {
                            // 版本选择下拉框
                            let selected_vol_name = self
                                .easy_mode_selected_volume
                            ...
                            let button =
                                egui::Button::new(egui::RichText::new(tr!("开始安装")).strong());
    ...
    fn draw_system_logo(
        ...
        if !self.easy_mode_logo_loading.contains(logo_url) {
            ...
            self.easy_mode_system_logo_cache
                .insert(logo_url.to_string(), EasyModeLogoState::Loading);
            ...
            std::thread::spawn(move || {
                ...
                LOGO_LOAD_RESULTS
                    .lock()
                    .unwrap()
                    .push(LogoLoadResult { url, data: result });
            });
        }
            ...
            match result.data {
                Ok(data) => {
                    ...
                    if let Ok(image) = image::load_from_memory(&data) {
                        ...
                        self.easy_mode_system_logo_cache
                            .insert(result.url, EasyModeLogoState::Loaded(texture));
                    } else {
                        self.easy_mode_system_logo_cache
                            .insert(result.url, EasyModeLogoState::Failed);
                    }
                }
                Err(_) => {
                    self.easy_mode_system_logo_cache
    ...
    fn show_easy_mode_confirm_dialog(
        ...
        let selected_system = self
            .easy_mode_selected_system
            .and_then(|idx| systems.get(idx));
        let selected_volume = selected_system.and_then(|(_, sys)| {
            self.easy_mode_selected_volume
                .and_then(|idx| sys.volume.get(idx))
        });
        ...
        egui::Window::new(tr!("确认重装系统"))
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add_space((window_width - text_width) / 2.0 - 16.0);
                    ui.label(
                        egui::RichText::new(tr!("将自动应用以下优化："))
                            .small()
                            .strong(),
                    );
                });
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add(confirm_btn).clicked() {
                        self.easy_mode_show_confirm_dialog = false;
                        self.start_easy_mode_install(system_name, system, volume.number);
                    }
                });
    ...
    fn start_easy_mode_install(
        ...
        // 设置用户名
        let username =
            crate::core::app_config::get_current_username().unwrap_or_else(|| "User".to_string());
