...
fn get_appx_packages_offline(target_partition: &str) -> Vec<AppxPackageInfo> {
    ...
    for entry in entries.flatten() {
        ...
        // 跳过特殊目录
        if dir_name == "Deleted"
            || dir_name == "DeletedAllUserPackages"
            || dir_name == "Merged"
            || dir_name == "Mutable"
            || dir_name == "MutableBackup"
            || dir_name == "Projected"
...
fn is_invalid_display_name(name: &str) -> bool {
    ...
    if words.len() == 1 && name.len() >= 5 && name.len() <= 8 {
        ...
        let known_names = [
            "Claude", "Cortana", "Spotify", "Discord", "Netflix", "Twitter", "Notepad", "Photos",
            "Camera", "Weather", "Calendar", "Music", "Video", "Office", "Edge",
        ];
            ...
            Ok(operation) => {
                match operation.get() {
                    ...
                    Err(e) => {
                        ...
                        // 尝试保留数据移除
                        if let Ok(op2) = pm.RemovePackageWithOptionsAsync(
                            &hstring_name,
