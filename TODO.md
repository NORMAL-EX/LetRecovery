...
- [ ] 修复浅色 Mica 下单行 Edit 已输入文字变成近白色的问题；必须与普通浅色按钮文字保持同等可读的深色前景，同时保留原生光标、选择、IME 和无障碍行为。
- [ ] 修复浅色 Mica 下闭合 ComboBox 的下拉箭头变成近白色、与字段背景失去对比的问题；正文、箭头和圆角外框必须使用同一最终表面状态。
...
- [ ] 修复从高级选项返回安装页后镜像卷下拉框与“选择安装分区”标题重叠的布局错误。
  - 稳定复现：进入高级选项后直接保存返回即错位；取消勾选再重新勾选“无人值守”会触发正确重排并暂时恢复，再次进入并退出高级选项后错误重现。返回路径必须复用无人值守可见性变化后的同一布局归一函数。
